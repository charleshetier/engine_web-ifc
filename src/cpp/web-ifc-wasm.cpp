...
std::optional<webifc::geometry::IfcMeshesIterator> ifcMeshIterator = std::nullopt;
...
void CreateAllMeshesStreamingSession(uint32_t modelID)
{
    if (!manager.IsModelOpen(modelID))
        return;

    if (ifcMeshIterator.has_value())
    {
        ifcMeshIterator.reset();
    }

    ifcMeshIterator = webifc::geometry::IfcMeshesIterator(manager, modelID);
}

webifc::geometry::IfcMeshesIteratorItem AllMeshesStreamingSessionNext()
{
    if (!ifcMeshIterator.has_value())
    {
        webifc::geometry::IfcMeshesIteratorItem item;
        item.Done = true;
        return item;
    }

    return ifcMeshIterator.value().Next();
}
...
EMSCRIPTEN_BINDINGS(my_module)
{
    ...
    emscripten::value_object<webifc::geometry::IfcMeshesIteratorItem>("IfcMeshesIteratorItem")
        .field("value", &webifc::geometry::IfcMeshesIteratorItem::Value)
        .field("done", &webifc::geometry::IfcMeshesIteratorItem::Done);
